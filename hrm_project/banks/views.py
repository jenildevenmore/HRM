...
class BankAccountViewSet(_BankAccessMixin, viewsets.ModelViewSet):
    serializer_class = BankAccountSerializer
    permission_classes = [IsAuthenticated]
    filter_backends = [DjangoFilterBackend, SearchFilter, OrderingFilter]
    filterset_fields = ['employee', 'is_primary', 'is_active']
    search_fields = ['bank_name', 'account_holder_name', 'account_number', 'ifsc_code', 'upi_id', 'employee__first_name', 'employee__last_name']
    ordering_fields = ['employee_id', 'bank_name', 'created_at']

    def get_queryset(self):
        qs = BankAccount.objects.select_related('client', 'employee')
